...
async function realCreateUploadUrl(eventSlug, deviceId, ext, caption) {
  const r = await fetch('/api/create-upload-url', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ eventSlug, deviceId, ext, caption })
  });
  if (!r.ok) {
    const t = await r.json().catch(() => ({}));
    const err = new Error(t.error || 'upload_error'); err.status = r.status; throw err;
  }
  return r.json();
}

async function realRecordPhoto(eventSlug, payload) {
  const r = await fetch('/api/record-photo', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ eventSlug, deviceId: getOrCreateDeviceId(), ...payload })
  });
...
export async function getEventBySlug(slug) {
  if (USE_REAL) return realGetEventBySlug(slug);
  const ev = ensureEvent(slug);
  seedPhotos(ev.id);
  return { ...ev, now: nowIso() };
}

export async function createUploadUrl(eventSlug, deviceId, ext = 'jpg', caption = '') {
  if (USE_REAL) return realCreateUploadUrl(eventSlug, deviceId, ext, caption);
  ...
  return { uploadUrl: `mock://${objectPath}`, objectPath, caption };
}

export async function recordPhoto(eventSlug, { objectPath, width, height, dataUrl, caption }) {
  if (USE_REAL) return realRecordPhoto(eventSlug, { storagePath: objectPath, width, height, caption });
  const ev = ensureEvent(eventSlug);
  const photo = { id: uuid(), eventId: ev.id, storagePath: objectPath, thumbUrl: dataUrl, fullUrl: dataUrl, width, height, createdAt: nowIso(), status: 'ready', caption: caption || null };
